...
class CircleProgressBar @JvmOverloads constructor(
    ...
    private fun parseAttribute(context: Context, attrs: AttributeSet) {
        ...
        ringWidth = ta.getDimension(R.styleable.CircleProgressBar_ringWidth,
        ...
        descTextSize =
            ta.getDimension(R.styleable.CircleProgressBar_descTextSize,
                DEFAULT_DESC_TEXT_SIZE.toFloat()
            ).toInt()
        descTextColor =
        ...
        percentTextSize = ta.getDimension(
            ...
            DEFAULT_PERCENT_TEXT_SIZE.toFloat()
        ).toInt()
        percentTextColor =
    ...
    private fun measure(measureSpec: Int): Int {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            result = SizeUtil.dp2px(context,90f).toInt()
    ...
    private fun calculateOffset(): Float {
        ...
        if (currentPercent + roundPercent >= 1.0f) {
            return startAngle
        } else if (currentPercent + 2 * roundPercent >= 1.0f) {
//            mArcPaint.strokeCap = Paint.Cap.BUTT
            if (reverse) {
                return startAngle + (1 - currentPercent - roundPercent) * 360f
            } else {
                return startAngle - (1 - currentPercent - roundPercent) * 360f
            }
        } else {
